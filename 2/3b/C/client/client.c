...
#define CLIENT_HOSTNAME "z35_c_client__2_3"  // IPv6 loopback address
#define CLIENT_PORT 8001
#define SERVER_HOSTNAME "z35_python_server_2_3"  // IPv6 loopback address
#define SERVER_PORT 8000
#define MESSAGE_SIZE 1000

int main() {
    int clientSocketFD;
    struct sockaddr_in6 serverAddress;  // Use sockaddr_in6 for IPv6
    int bytesSent;
    int bytesReceived;
    ...
    clientSocketFD = createStreamSocketIPv6(CLIENT_PORT);

    // Set up server address for IPv6
    constructSocketAddressIPv6(&serverAddress, SERVER_HOSTNAME, 8000);
    char ipAddress[INET6_ADDRSTRLEN];  // Use INET6_ADDRSTRLEN for IPv6

    // Convert binary IP address to string representation
    if (inet_ntop(AF_INET6, &(serverAddress.sin6_addr), ipAddress, INET6_ADDRSTRLEN) == NULL) {
        perror("inet_ntop");
        exit(EXIT_FAILURE);
    }
    int port = ntohs(serverAddress.sin6_port);
    printf("C client\n");
